"""
alignment_small.py (OPTIMIZED)
=====================
Modulo per l'allineamento verticale dei tappi (tappo trasparente piccolo).
...
# Parametri Hough di default calibrati per il tappo trasparente PICCOLO
# (diversi da quelli di alignment.py, calibrati sul tappo grande).
_SMALL_MIN_RADIUS = 90
_SMALL_MAX_RADIUS = 120
_SMALL_MIN_DIST = 500
_SMALL_PARAM1 = 50
_SMALL_PARAM2 = 30
...
def find_cap_center_via_markers(
    ...
    min_radius: int = _SMALL_MIN_RADIUS,
    max_radius: int = _SMALL_MAX_RADIUS,
    min_dist: int = _SMALL_MIN_DIST,
    param1: float = _SMALL_PARAM1,
    param2: float = _SMALL_PARAM2,
    pair_dist_range: tuple[float, float] = (700.0, 860.0),
    center_dist_range: tuple[float, float] = (300.0, 470.0),
) -> tuple[float, float] | None:
    """
    Stima il centro del tappo (trasparente piccolo) come punto medio tra i
    due marker fisici rilevati via Hough — stessa logica di
    align_cap_image_small, isolata qui per essere riusata da
    augment_dataset.py (che ha bisogno solo del centro per ruotare attorno
    al punto giusto, non dell'allineamento/angolo finale).
...
def align_cap_image_small(img: np.ndarray, template: np.ndarray) -> np.ndarray:
    """
    Ruota un'immagine del tappo in memoria in modo che l'asse sia allineato
    verticalmente.
    """
    ...
    pair = _find_marker_pair(
        gray,
        min_radius=_SMALL_MIN_RADIUS, max_radius=_SMALL_MAX_RADIUS,
        min_dist=_SMALL_MIN_DIST, param1=_SMALL_PARAM1, param2=_SMALL_PARAM2,
    )
    ...
    base_rotation: float = current_angle + 90  # confermato: diverso da alignment.py per questo tappo
    ...
    if score_flipped < score_upright:
        final_angle += 180
    #print(f"score upright: {score_upright:.4f}, score flipped: {score_flipped:.4f}, final_angle: {final_angle:.1f}")
    ...
    pt1: np.ndarray = np.dot(M_final, np.array([x1, y1, 1]))
    pt2: np.ndarray = np.dot(M_final, np.array([x2, y2, 1]))
    cv2.line(
        result,
        (int(pt1[0]), int(pt1[1])),
        (int(pt2[0]), int(pt2[1])),
